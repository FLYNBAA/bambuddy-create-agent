...
# Windows checkouts preserve CRLF in shell scripts. Linux reports the script
# as missing because its shebang resolves to `/bin/sh\r`; normalize inside the
# image so Docker Desktop and Linux builds use the same source tree.
