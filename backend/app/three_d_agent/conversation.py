...
class CreatorConversationPlanner:
    ...
    async def _invoke_model(self, session: SessionSnapshot, message: str) -> CreatorCommand:
        ...
        system = """You control a 3D-print creator workflow. Choose exactly one action.
prepare handles normal creative requests and revisions. Paid actions confirm_images,
confirm_3d, and generate_print_file require the user's unambiguous explicit confirmation;
set explicit_confirmation false otherwise. select_image needs image_index 0..3.
For generate_print_file, set acknowledge_issues true only when the newest user message
explicitly acknowledges the reported print-analysis issues or risks. Never infer that
acknowledgment from a request to continue.
                ...
                async with httpx.AsyncClient(
                    ...
                    **httpx_route_kwargs(trust_env),
                    ...
                    model = ChatOpenAI(
                        ...
                        http_socket_options=(),
                    ).with_structured_output(CreatorCommand, method="json_mode")
                    command = await model.ainvoke(
                        [("system", system), ("human", f"Workflow status: {session.status.value}\nHistory:\n{history}\nNewest user message: {message}")]
                    )
                return command if isinstance(command, CreatorCommand) else CreatorCommand.model_validate(command)
