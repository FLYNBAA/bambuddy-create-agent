...
async def _max_z_for_current_print(printer_id: int, data: dict, logger) -> float | None:
    ...
    try:
        ...
        raw_path = str(archive.file_path)
        path = Path(raw_path)
        if not path.is_absolute() and not PurePosixPath(raw_path).is_absolute():
...
async def lifespan(app: FastAPI):
    ...
    await init_db()
    from backend.app.services.creator_integration import CreatorController, load_creator_config_overrides

    creator_controller = CreatorController()
    async with async_session() as creator_db:
        overrides = await load_creator_config_overrides(creator_db)
    if overrides:
        creator_controller.reconfigure(**overrides)
    recovered_creator_sessions = creator_controller.agent.recover_interrupted_sessions()
    if recovered_creator_sessions:
        logging.getLogger(__name__).warning("Recovered %s interrupted BCA creator session(s)", recovered_creator_sessions)
    app.state.bca_creator = creator_controller
    ...
    yield
    creator_controller = getattr(app.state, "bca_creator", None)
    if creator_controller is not None:
        await creator_controller.shutdown()
