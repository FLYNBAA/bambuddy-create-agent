...
class TestLibraryPathHelpers:
    ...
    def test_to_absolute_path_converts_relative(self):
        ...
        assert abs_path.as_posix() == f"{settings.base_dir.as_posix()}/archive/library/files/test.3mf"

    def test_to_absolute_path_handles_already_absolute(self):
        """Native absolute legacy paths are returned as-is."""
        from backend.app.api.routes.library import to_absolute_path

        if os.name == "nt":
            with pytest.raises(ValueError, match="escapes base directory"):
                to_absolute_path("/data/archive/test.3mf")
            return
        ...
        assert result.as_posix() == abs_path_str
