...
def _write_rotation(log_dir, live: str, backups: list[str]):
    ...
    """
    (log_dir / "bambuddy.log").write_bytes(live.encode("utf-8"))
    for index, text in enumerate(backups, start=1):
        (log_dir / f"bambuddy.log.{index}").write_bytes(text.encode("utf-8"))
...
def test_partial_line_at_the_truncation_point_is_discarded(tmp_path):
    ...
    (tmp_path / "bambuddy.log").write_bytes(b"aaaaaaaaaa\nbbbbbbbbbb\n")
...
def test_missing_backups_are_skipped_not_fatal(tmp_path):
    (tmp_path / "bambuddy.log").write_bytes(b"live only\n")
    (tmp_path / "bambuddy.log.2").write_bytes(b"older\n")  # .log.1 absent
