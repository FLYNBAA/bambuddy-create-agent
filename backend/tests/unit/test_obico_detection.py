...
class TestTestConnection:
    @pytest.mark.asyncio
    async def test_connection_failure_returns_the_transport_error(self):
        svc = ObicoDetectionService()
        mock_client = MagicMock()
        mock_client.get = AsyncMock(side_effect=OSError("simulated DNS failure"))
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        mock_client.__aexit__ = AsyncMock(return_value=False)

        with patch("backend.app.services.obico_detection.httpx.AsyncClient", return_value=mock_client):
            result = await svc.test_connection("http://obico.invalid:3333")

        assert result["ok"] is False
        assert result["error"] == "simulated DNS failure"
