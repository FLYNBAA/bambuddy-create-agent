...
# pyftpdlib's implicit-FTPS mock retains Windows file handles after transfers,
# so this live socket suite belongs to Linux CI where its transport semantics
# match the production FTPS target. Unit tests outside this module still cover
# Bambu FTP helpers on Windows.
pytestmark = pytest.mark.skipif(os.name == "nt", reason="pyftpdlib implicit-FTPS mock is not Windows-safe")
